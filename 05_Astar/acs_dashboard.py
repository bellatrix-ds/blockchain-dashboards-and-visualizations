...
# ── PAGE CONFIG ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="ACS Campaign Report — Bella",
    page_icon="📊",
    layout="wide",
...
.eyebrow {{
    ...
    letter-spacing: 0.14em; color: {BLUE}; margin-bottom: 8px;
}}
.heading {{
...
.body-text {{
    font-size: 15px; color: {MUTED}; line-height: 1.75;
    max-width: 800px; margin-bottom: 36px;
}}
.body-white {{
    font-size: 15px; color: {WHITE}; line-height: 1.75;
    max-width: 800px; margin-bottom: 36px;
}}
.col-title {{
...
.kpi-label {{
    font-size: 11px; font-weight: 700; text-transform: uppercase;
    letter-spacing: 0.1em; color: {MUTED}; margin-bottom: 10px;
}}
.kpi-number {{
...
.insight-label {{
    ...
    letter-spacing: 0.1em; color: {CYAN}; margin-bottom: 8px;
}}
.rec-card {{
    background: {CARD}; border: 1px solid {BORDER};
...
.rec-num {{
    font-size: 11px; font-weight: 700; text-transform: uppercase;
    letter-spacing: 0.1em; color: {BLUE}; margin-bottom: 8px;
}}
.rec-title {{
...
.divider {{ border: none; border-top: 1px solid {BORDER}; margin: 36px 0; }}
</style>
...
# ══════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    # Try to load profile image from data folder
    img_path = os.path.join(DATA_DIR, "profile.jpg")
    img_url  = f"{GITHUB}/profile.jpg"
    ...
    if os.path.exists(img_path):
        st.image(img_path, width=140, use_column_width=False)
    else:
        # Placeholder avatar circle if no image found
        st.markdown(f"""
        <div style="width:120px; height:120px; border-radius:12px;
                    background:linear-gradient(135deg,{BLUE}33,{PINK}33);
                    border:2px solid {BORDER}; margin:0 auto 0;
    ...
    <div style="padding: 20px 24px 0; text-align: left;">
      <div style="font-size:20px; font-weight:800; color:{WHITE}; margin-bottom:4px;">
        My name is Bella
      </div>
      <div style="font-size:13px; color:{MUTED}; margin-bottom:24px; line-height:1.5;">
        Blockchain Research Analyst
      </div>
      <div style="font-size:12px; font-weight:700; text-transform:uppercase;
                  letter-spacing:0.1em; color:{MUTED}; margin-bottom:14px;">
        Get in touch 👇
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="padding: 0 24px;">
      <a href="#" style="{btn_style}">🗂  Portfolio</a>
      <a href="https://github.com/bellatrix-ds" target="_blank" style="{btn_style}">🐙  GitHub</a>
      <a href="https://x.com" target="_blank" style="{btn_style}">𝕏  x.com</a>
      <a href="mailto:bella@example.com" style="{btn_style}">✉️  Email</a>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="padding: 28px 24px 0;">
      <hr style="border:none; border-top:1px solid {BORDER}; margin-bottom:20px;">
    ...
    for s in sections:
        st.markdown(f"""
        <div style="padding:0 24px;">
          <div style="font-size:12px; color:{MUTED}; padding:7px 0;
    ...
    <div style="padding:24px 24px 36px;">
      <div style="font-size:10px; color:{MUTED}; line-height:1.7; margin-top:8px;">
        Data: DeFiLlama, Blockscout,<br>Astar Forum, AFC Reports<br>
        <span style="color:{BORDER};">Analysis as of April 2026</span>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
# ── DASHBOARD TITLE ───────────────────────────────────────────────────────────
st.markdown(f"""
<div style="background:{SURFACE}; border-bottom:1px solid {BORDER}; padding:36px 52px 28px;">
  <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:16px;">
    <div>
      <div style="font-size:11px; font-weight:700; text-transform:uppercase;
                  letter-spacing:0.14em; color:{BLUE}; margin-bottom:8px;">
        Astar Network · Post-Mortem Analysis
      </div>
      <div style="font-size:32px; font-weight:800; color:{WHITE}; margin-bottom:8px; line-height:1.2;">
        ACS Campaign Performance Report
      </div>
      <div style="font-size:14px; color:{MUTED};">
        Feb 20 – May 30, 2025 &nbsp;·&nbsp; Soneium Ecosystem &nbsp;·&nbsp; Analysis as of April 2026
      </div>
    </div>
    <div style="display:flex; gap:8px; flex-wrap:wrap; padding-top:4px;">
      <span style="background:rgba(16,185,129,0.12); color:{GREEN}; border:1px solid rgba(16,185,129,0.25); font-size:11px; font-weight:700; padding:5px 14px; border-radius:5px; letter-spacing:0.06em;">PRE-ACS</span>
      <span style="background:rgba(0,180,255,0.12);  color:{BLUE};  border:1px solid rgba(0,180,255,0.25);  font-size:11px; font-weight:700; padding:5px 14px; border-radius:5px; letter-spacing:0.06em;">ACS ACTIVE</span>
      <span style="background:rgba(245,158,11,0.12); color:{AMBER}; border:1px solid rgba(245,158,11,0.25); font-size:11px; font-weight:700; padding:5px 14px; border-radius:5px; letter-spacing:0.06em;">POST-ACS</span>
      <span style="background:rgba(239,68,68,0.12);  color:{RED};   border:1px solid rgba(239,68,68,0.25);  font-size:11px; font-weight:700; padding:5px 14px; border-radius:5px; letter-spacing:0.06em;">CURRENT</span>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
...
<div class="section-wrap">
  <div class="eyebrow">01 — Campaign Summary</div>
  <div class="heading">What happened, and what didn't.</div>
  <div class="body-white">
    The ACS campaign delivered impressive headline numbers — $226M TVL, 3.57M wallets,
    147M transactions. But 95.8% of that TVL vanished within 90 days of the reward claim date.
    This report quantifies what stuck, what left, and what Astar should do differently next time.
  </div>
</div>
""", unsafe_allow_html=True)
...
with st.container():
    ...
    st.markdown(f"""<div style="margin-bottom:20px;">
      <div class="eyebrow" style="margin-bottom:6px;">Three Verdicts</div>
      <div class="subheading" style="margin-bottom:24px;">What the data tells us</div>
    </div>""", unsafe_allow_html=True)
...
st.markdown(f"""
<div class="section-wrap">
  <div class="eyebrow">02 — TVL & Activity</div>
  <div class="heading">The campaign created a mountain, not a foundation.</div>
  <div class="body-text">TVL grew 267% during ACS. It collapsed 96% after. The charts below show exactly when activity became artificial — and what the organic baseline actually looks like.</div>
</div>
""", unsafe_allow_html=True)

with st.container():
    st.markdown("<div style='padding: 0 52px 52px;'>", unsafe_allow_html=True)
    ...
    fig_tvl.add_trace(go.Scatter(
        ...
        line=dict(color=PINK, width=3), name="Total TVL ($M)",
        hovertemplate="<b>%{x}</b><br>TVL: $%{y:.0f}M<extra></extra>", mode="lines",
    ))
    fig_tvl.add_annotation(x="S9", y=220, text="<b>Peak $226M</b>",
        showarrow=True, arrowhead=2, arrowcolor=PINK,
        font=dict(color=PINK, size=13), ax=40, ay=-36)
    fig_tvl.add_annotation(x="Apr'26", y=9.5, text="<b>$9.5M today</b>",
        showarrow=False, font=dict(color=RED, size=13), xanchor="right", yshift=16)
    ...
    When rewards ended May 30, SolvBTC.BBN exited –62% within 30 days of the June 10 claim date, accounting for the majority of the $216M collapse. The organic floor was always ~$9–12M.</div>""", unsafe_allow_html=True)
...
st.markdown(f"""
<div class="section-wrap">
  <div class="eyebrow">03 — Protocol Autopsy</div>
  <div class="heading">Which protocols built something real?</div>
  <div class="body-text">Six protocols received significant ACS allocation. Below is what each looked like at peak vs. today — and an honest verdict on whether ACS helped them build a lasting user base or just rented temporary attention.</div>
</div>
""", unsafe_allow_html=True)
...
st.markdown(f"""
<div class="section-wrap">
  <div class="eyebrow">04 — ASTR Token</div>
  <div class="heading">Did ASTR earn its place on Soneium?</div>
  <div class="body-text">Astar's core goal was to make ASTR the economic engine of Soneium — used as collateral, gas, and governance. The bridge flow and asset retention data shows whether that goal was achieved.</div>
</div>
""", unsafe_allow_html=True)
...
with st.container():
    ...
    with c_as:
        st.markdown("<div class='col-title'>Asset Retention — 30 Days After Claim Date</div>", unsafe_allow_html=True)
        ...
        st.markdown(f"""<div class="caption"><b>What stayed:</b> SolvBTC.BBN (–62%) drove the collapse. USDC (+0.9%) and liquid staking (wstASTR +19.8%, vASTR +2.7%) were the stickiest — the only genuine ASTR utility signal in this dataset.</div>""", unsafe_allow_html=True)
...
st.markdown(f"""
<div class="section-wrap">
  <div class="eyebrow">05 — Gaming vs DeFi</div>
  <div class="heading">Gaming keeps users. DeFi rents them.</div>
  <div class="body-text">Astar's pitch to Sony was "we bring gamers onchain." The retention data supports this — but only partially. Gaming protocols outperform DeFi on retention, but even the best gaming protocol falls well below what successful gaming chains achieve.</div>
</div>
""", unsafe_allow_html=True)
...
with st.container():
    ...
    with cg2:
        ...
        for color, title, body in [
            ...
            (CYAN,   "The key strategic insight",    "Mechanics creating non-financial goals (collecting, ranking, rare items) retain users at 2–8× the rate of financial-only mechanics. Require gaming protocols to demonstrate a non-financial retention hook before receiving ACS 2.0 allocation."),
        ]:
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 6 — L2 BENCHMARK
# ══════════════════════════════════════════════════════════════════════════════
st.markdown(f"""
<div class="section-wrap">
  <div class="eyebrow">06 — L2 Benchmark</div>
  <div class="heading">Soneium's retention is 4× below the L2 average.</div>
  <div class="body-text">Incentive campaigns are common across L2s. But Soneium's 4.2% post-incentive TVL retention is significantly below Base (35%), Linea (22%), and Blast (18%). Here's why — and what those chains did differently.</div>
</div>
""", unsafe_allow_html=True)
...
with st.container():
    ...
    for col, color, title, body in [
        (b1, GREEN, "Base — 35% Retention", "Coinbase's 100M+ user base gave Base structural distribution. Incentivized users were already Coinbase customers with existing crypto behavior — not first-time participants attracted by yield."),
        (b2, AMBER, "Blast — 18% Retention", "Blast's native yield model (ETH and stablecoin auto-yield) means the chain generates returns independent of protocol incentives — a baseline reason to keep assets on-chain after campaigns end."),
        (b3, BLUE,  "Linea — 22% Retention", "MetaMask integration (1B+ installs) provided organic user access. Campaign participants already had Web3 infrastructure — not mercenary capital attracted purely by ASTR yields."),
    ]:
        with col:
            st.markdown(f"""<div class="card">
...
st.markdown(f"""
<div class="section-wrap">
  <div class="eyebrow">07 — Insights & Recommendations</div>
  <div class="heading">What Astar doesn't know yet — and what to do about it.</div>
  <div class="body-text">Four findings not published by Astar. Three recommendations their growth team can act on immediately.</div>
</div>
""", unsafe_allow_html=True)

with st.container():
    st.markdown("<div style='padding: 0 52px 72px;'>", unsafe_allow_html=True)
    st.markdown(f"<div class='col-title' style='margin-bottom:20px;'>New Insights — Not Published by Astar</div>", unsafe_allow_html=True)
    ...
    for num, title, body in [
        ...
        ("04", "Path of Soneium Received Gaming Rewards Without a Game",
         "The discretionary gaming allocation included Path of Soneium — a quest checklist app with no game loop, no competitive mechanic, and no reason for return visits. It received 2M ASTR/season — equivalent to Evermoon — despite a fraction of the retention. The gaming allocation had no minimum engagement criteria."),
    ]:
    ...
    st.markdown("<div style='height:40px'></div>", unsafe_allow_html=True)
    st.markdown(f"<div class='col-title' style='margin-bottom:20px;'>Three Recommendations for Astar's Growth Team</div>", unsafe_allow_html=True)
    ...
    for col, num, timeline, title, body, metric in [
        (r1, "Recommendation 01", "Protocol + AFC · Q3 2026",
         ...
         "SolvBTC.BBN exited –62% within 30 days — the majority of ACS peak TVL. A 90-day lock requiring ≥15% TVL retention as a prerequisite for full payout would have redirected ~$130M in phantom TVL incentives toward protocols with genuine organic floors.",
        ...
        (r2, "Recommendation 02", "Growth team + Sentio · Q2–Q3 2026",
         ...
         "Yoki Legacy's collection mechanic retained users at 2× Evermoon's rate with similar allocation. Path of Soneium received gaming rewards with zero game loop. A 3-metric behavioral dashboard (30-day return rate, sessions per retained wallet, non-financial in-app actions) would redirect incentives toward genuinely sticky products.",
        ...
        (r3, "Recommendation 03", "Business dev + Startale Labs · Q4 2026",
         ...
         "Soneium's gaming retention (6–13%) sits below comparable gaming chains (15–40%) because no ACS protocol had Sony IP access. Startale Labs' $63M Series A provides negotiating leverage. Gate ACS 2.0 gaming allocation on at least one signed Sony IP product being live at launch.",
    ...
    st.markdown(f"""<div style="margin-top:52px;padding-top:20px;border-top:1px solid {BORDER};font-size:12px;color:{MUTED};line-height:1.9;">
